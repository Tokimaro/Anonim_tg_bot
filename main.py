...
def search_prof(message):
    if (message.text == u'Начать поиск') or (message.text == u'Посмотреть профиль') or (message.text == u'Удалить профиль'):
...
def search_partner(message):
    open = check_open(first_id=message.from_user.id)
    if open[0][0]:
        print("d")
        ...
        if not select:
            ...
            for sel in select:
                ...
                print(sel[0])
                print(message.from_user.id == sel[0])
                if not check_status(first_id=message.from_user.id, second_id=sel[0]) or message.from_user.id == sel[0]:
                    ...
                    bot.send_message(message.from_user.id,"Имя собеседника: " + str(user_info[2]) + "\nВозраст собеседника: " + str(
